...
# 面向定制报告工作台的细分模块。每个模块复用已验证的章节引擎，再用指标白名单
# 收窄到具体分析范围，避免把「偿债能力」等模块退化成整套财务报告。
CUSTOM_MODULE_REGISTRY: dict[str, dict[str, Any]] = {
    "financial_overview": {
        "title": "财务总览",
        "desc": "资产、负债、盈利、现金流与成长综合体检",
        "base_chapter": "financial",
        "category": "财务分析",
        "custom_visible": True,
        "keywords": ["财务总览", "财务健康", "财务体检"],
    },
    "solvency": {
        "title": "偿债能力",
        "desc": "资产负债率、流动比率与速动比率",
        "base_chapter": "financial",
        "category": "财务分析",
        "custom_visible": True,
        "metric_allowlist": ["debt_ratio", "current_ratio", "quick_ratio"],
        "keywords": ["偿债能力", "偿债", "短债"],
    },
    "profitability": {
        "title": "盈利能力",
        "desc": "毛利率、净利率、ROE 与 ROA",
        "base_chapter": "financial",
        "category": "财务分析",
        "custom_visible": True,
        "metric_allowlist": ["gross_margin", "net_margin", "roe", "roa", "profit_margin"],
        "keywords": ["盈利能力", "盈利", "利润率"],
    },
    "cashflow": {
        "title": "现金流质量",
        "desc": "经营、投资、筹资现金流与造血能力",
        "base_chapter": "financial",
        "category": "财务分析",
        "custom_visible": True,
        "metric_allowlist": ["operating_cf", "investing_cf", "financing_cf", "cash_flow_level", "cash_flow_net"],
        "keywords": ["现金流质量", "现金流", "造血"],
    },
    "growth": {
        "title": "成长能力",
        "desc": "营收与利润同比、增长质量",
        ...
        "category": "财务分析",
        "custom_visible": True,
        "metric_allowlist": ["revenue_yoy", "profit_yoy", "avg_revenue_yoy", "avg_revenue_yoy_spread"],
        "keywords": ["成长能力", "成长", "同比"],
    },
    "tax_compliance": {
        "title": "税务合规",
        ...
        "category": "税务分析",
        "custom_visible": True,
        "metric_allowlist": ["tax_on_time_rate", "tax_arrears_cnt", "tax_late_penalty_cnt", "tax_violation_cnt"],
        "keywords": ["税务合规", "纳税合规", "欠税"],
    },
    "tax_burden": {
        "title": "税负结构",
        "desc": "增值税与所得税税负率",
        ...
        "category": "税务分析",
        "custom_visible": True,
        "metric_allowlist": ["vat_burden", "income_tax_burden"],
        "keywords": ["税负结构", "税负", "增值税税负", "所得税税负"],
    },
    "invoice_fraud": {
        "title": "发票舞弊线索",
        "desc": "进销错配、红冲与异常凭证",
        ...
        "category": "发票与交易",
        "custom_visible": True,
        "metric_allowlist": ["flagged_count", "signal_count", "fraud_composite_score", "scbm_mismatch_rate", "red_anomaly_ratio"],
        "keywords": ["发票舞弊", "税票舞弊", "异常凭证", "红冲"],
    },
    "invoice_concentration": {
        "title": "上下游集中度",
        "desc": "客户、供应商与交易集中风险",
        "base_chapter": "fraud",
        "category": "发票与交易",
        "custom_visible": True,
        "metric_allowlist": ["customer_hhi", "supplier_hhi", "top_customer_share", "customer_top5_concentration", "category_concentration"],
        "keywords": ["上下游集中", "客户集中", "供应商集中", "交易集中"],
    },
    "revenue_authenticity": {
        "title": "收入真实性",
        "desc": "申报、开票、财报多源收入一致性",
        "base_chapter": "authenticity",
        "category": "真实性核验",
        "custom_visible": True,
        "metric_allowlist": ["suspicious_count", "benford_mad", "revenue_deviation", "compare_spread", "authenticity_score"],
        "keywords": ["收入真实性", "经营真实性", "多源营收", "收入核验"],
    },
    "risk_signals": {
        "title": "风险信号总览",
        "desc": "税务违法、信用、偏差与多重叠加信号",
        "base_chapter": "signal",
        "category": "风险与评级",
        "custom_visible": True,
        "metric_allowlist": ["signal_total", "multi_hit_count", "low_credit_industry_count", "high_dev", "low_credit", "tax_violation"],
        "keywords": ["风险信号", "风险预警", "异常信号"],
    },
    "credit_rating": {
        "title": "信用与经营画像",
        "desc": "信用等级、综合评分与六维经营表现",
        "base_chapter": "score",
        "category": "风险与评级",
        "custom_visible": True,
        "metric_allowlist": ["avg_credit_score", "overall_score", "avg_score"],
        "metric_allow_prefixes": ["dim_"],
        "keywords": ["信用评级", "信用等级", "经营画像", "综合评分"],
    },
    "peer_benchmark": {
        "title": "行业基准对标",
        "desc": "同业均值、分位与偏离程度",
        "base_chapter": "benchmark",
        "category": "对比与趋势",
        "custom_visible": True,
        "metric_allowlist": ["peer_industry_percentile", "peer_province_percentile"],
        "keywords": ["行业基准", "同业对标", "同行均值", "分位"],
    },
    "industry_trend": {
        "title": "行业营收趋势",
        "desc": "行业营收同比、增长分化与变化方向",
        "base_chapter": "trend",
        "category": "对比与趋势",
        "custom_visible": True,
        "metric_allowlist": ["avg_revenue_yoy", "avg_revenue_yoy_spread", "revenue_yoy", "profit_yoy"],
        "keywords": ["行业趋势", "营收趋势", "行业增速"],
    },
}
